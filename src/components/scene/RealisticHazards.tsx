...
const clampFloorY = (hazard: Hazard) => {
  ...
  // Clamp all floor-based hazards to the slab.
...
const StackingHazard = ({ hazard, identified, onIdentify, showHint }: RealisticHazardProps) => {
  ...
  return (
    <group ref={groupRef} position={[hazard.x, y, hazard.z]}>
      {/* Pallet base */}
...
// Dock edge missing safety signage (A-frame sign + edge marking)
const DockEdgeSignageHazard = ({ hazard, identified, onIdentify, showHint }: RealisticHazardProps) => {
  ...
  return (
    ...
    >
      {/* Dock edge stripe on floor */}
      <mesh rotation={[-Math.PI / 2, 0, 0]} position={[0, 0.006, 0]}>
        <planeGeometry args={[2.6, 0.22]} />
        <meshStandardMaterial color={identified ? '#22c55e' : '#fbbf24'} roughness={0.7} />
      </mesh>

      {/* A-frame sign */}
      <group position={[0.5, 0, 0.3]} rotation={[0, -0.6, 0]}>
        <RoundedBox args={[0.45, 0.65, 0.03]} position={[0, 0.55, 0.12]} radius={0.02}>
        ...
        {/* hinge */}
        <Cylinder args={[0.02, 0.02, 0.5, 10]} position={[0, 0.88, 0]} rotation={[0, 0, Math.PI / 2]}>
          <meshStandardMaterial color="#1f2937" metalness={0.6} roughness={0.4} />
        </Cylinder>
        {/* feet */}
        {[-0.18, 0.18].map((x, i) => (
          <Box key={i} args={[0.08, 0.03, 0.22]} position={[x, 0.015, 0.18]}>
            <meshStandardMaterial color="#111827" roughness={0.7} />
          </Box>
        ))}
      </group>

      {/* Missing wall sign bracket (suggests signage should be installed) */}
      <group position={[-0.9, 1.4, -0.35]}>
        <Box args={[0.35, 0.25, 0.02]}>
          <meshStandardMaterial color={identified ? '#22c55e' : '#374151'} transparent opacity={0.5} />
        </Box>
        <Cylinder args={[0.015, 0.015, 0.22, 8]} position={[-0.18, 0, 0]} rotation={[Math.PI / 2, 0, 0]}>
      ...
      {showHint && !identified && <pointLight position={[0.2, 0.6, 0.2]} color="#ef4444" intensity={4} distance={5} />}
    </group>
  );
};
